...
class ItemRequestServiceImplTest {
    ...
    void beforeEach() {
        ...
        itemRequest2 = new ItemRequest(1L, "Request",LocalDateTime.now(), UserMapper.toUser(userDto));
    ...
    void getAllRequest() {
        ...
        when(userService.getUser(userDto.getId())).thenReturn(userDto);
        when(itemRequestRepository.findAll()).thenReturn(Collections.singletonList(itemRequest2));
        when(itemRepository.findAllByOwnerId(userDto.getId(),pageable)).thenReturn(Collections.singletonList(item));
