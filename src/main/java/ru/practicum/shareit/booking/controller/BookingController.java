...
public class BookingController {
    ...
    public ResponseEntity<List<BookingResponseDto>> getAllBookingsByBookerId(
                                                    ...
                                                    @RequestHeader("X-Sharer-User-Id") Long userId) {
        return ResponseEntity.ok().body(bookingService.getAllBookingsByBookerId(state, userId));
    }
    ...
    public ResponseEntity<List<BookingResponseDto>> getAllBookingsOwnerItem(
                                                    @RequestParam(value = "state", defaultValue = "ALL") String state,
                                                    @RequestHeader("X-Sharer-User-Id") Long userId) {
        return ResponseEntity.ok().body(bookingService.getAllBookingsOwnerItem(state, userId));
    }
}